...
from google.adk.models.lite_llm import LiteLlm  # 新增导入
from google.genai import types
from google.adk.events import Event

# --- 设置Qwen API配置 ---
# 从环境变量获取API密钥，如果未设置则使用默认值（建议使用环境变量）
DASHSCOPE_API_KEY = os.getenv("API_KEY")
QWEN_API_BASE = "https://dashscope.aliyuncs.com/compatible-mode/v1"
QWEN_MODEL = "openai/qwen-plus"  # 可选模型：qwen-plus, qwen-turbo, qwen-max等
...
try:
    qwen_model = LiteLlm(
        ...
        temperature=0.1  # 可选参数，控制创造性
    )
    print(f"✅ Qwen模型初始化成功: {QWEN_MODEL}")
except Exception as e:
    print(f"❌ Qwen模型初始化失败: {e}")
    # 如果初始化失败，可以回退到其他模型或退出
    raise

## 定义配备各自工具的专门子 Agent
booking_agent = Agent(
    name="Booker",
    model=qwen_model,  # 使用Qwen模型替代Gemini
...
async def main():
    """运行 ADK 示例的主函数。"""
    print("--- Google ADK 路由示例（使用Qwen模型）---")
    print(f"使用模型: {QWEN_MODEL}")
    
    # 检查API密钥
    if DASHSCOPE_API_KEY == "您的DashScope API密钥":
        print("⚠️  警告：请设置正确的DASHSCOPE_API_KEY环境变量或修改代码中的默认值")
        return
